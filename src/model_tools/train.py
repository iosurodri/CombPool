                    ...
                    if param.weight is not None:
                        for coeff_idx, coeff in enumerate(param.weight):
                            ...
                            if coefficient.size == 1:
                                ...
                                writer.add_histogram('pool{}_coefficient{}'.format(pool_idx, coeff_idx), coefficient, 0)
                    pool_idx += 1
                            ...
                            if type(param) in (ChannelwiseCombPool2d, GatedCombPool2d): 
                                if param.weight is not None:
                                    for coeff_idx, coeff in enumerate(param.weight):
                                        coefficient = coeff.cpu().detach().numpy().squeeze()
                                        if coefficient.size == 1:
                                            writer.add_scalar('pool{}_coefficient{}'.format(pool_idx, coeff_idx), coefficient.item(), num_batches * epoch + (i + 1))
                                        else:
                                            writer.add_histogram('pool{}_coefficient{}'.format(pool_idx, coeff_idx), coefficient, num_batches * epoch + (i + 1))
                                pool_idx += 1
                    # if log_param_conv:
                        conv_idx = 0
                        for param in model.children():
